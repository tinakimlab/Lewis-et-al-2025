...
cairo_ps("fig 3b.eps", width = 5.2, height = 8, family = "ArialMT", pointsize = 12)
...
# Use Cairo EPS device
cairo_ps("fig 3c.eps", width = 5.2, height = 8, family = "ArialMT", pointsize = 12)
...
# Use Cairo EPS device
cairo_ps("fig 3d.eps", width = 5.2, height = 8, family = "ArialMT", pointsize = 12)
...
ggsave("fig 3g.eps", plot = combined_plot, device = cairo_ps, 
       path = "/Users/Elly/Desktop/Kim Lab-Whistler Lab/D3 Behavior Manuscript/EPS files",
       width = 10.4, height = 8, dpi = 300, family = "ArialMT")
...
#Figure 3H
```{r}
vehicle_mag <- data.frame(peak_magnitude) %>% filter(Treatment=="baselineandvehicle") %>% drop_na(period)
quet_mag <- data.frame(peak_magnitude) %>% filter(Treatment=="baselineandquetiapine15") %>% drop_na(period)

veh_summary <- vehicle_mag %>%
  group_by(period) %>%
  summarize(
    mean = mean(avg_mag, na.rm = TRUE),
    sem = sd(avg_mag, na.rm = TRUE) / sqrt(n()))

quet_summary <- quet_mag %>%
  group_by(period) %>%
  summarize(
    mean = mean(avg_mag, na.rm = TRUE),
    sem = sd(avg_mag, na.rm = TRUE) / sqrt(n()))

plot_veh <- ggplot(vehicle_mag, aes(x = period)) +
  geom_bar(aes(y = avg_mag, fill = period, color = period), 
           alpha = 0.5, 
           stat = "summary", 
           fun = "mean",
           width = 0.7) +
  ...
  scale_fill_manual(values = c("baseline"="white", "drug period"="gray"))+
  guides(fill = FALSE, color = FALSE) +  # Remove legend for fill and color 
  geom_point(aes(y = avg_mag, fill=period, color=period), size = 1.5) +
  scale_fill_manual(values = c("baseline"="white", "drug period"="gray46"))+
  scale_color_manual(values=c("baseline"="black", "drug period"="gray46")) +
  geom_path(aes(group = treatmentmouse, y = avg_mag), alpha = 0.8) +
  theme_classic() +
  ylab("Peak Magnitude (mean zscore)") +
  ...
  theme(
    ...
    axis.text.y = element_text(color = "black"),
        axis.line = element_line(size = 1.2)) 
# Plot for Quetiapine
plot_quet <- ggplot(quet_mag, aes(x = period)) +
   geom_bar(aes(y = avg_mag, fill = period, color = period), 
           alpha = 0.5, 
           stat = "summary", 
           fun = "mean",
...
# Export the plot to EPS
ggsave("fig 3h.eps", plot = combined_plot, device = cairo_ps, 
       path = "/Users/Elly/Desktop/Kim Lab-Whistler Lab/D3 Behavior Manuscript/EPS files",
       width = 10.4, height = 8, dpi = 300, family = "ArialMT")
